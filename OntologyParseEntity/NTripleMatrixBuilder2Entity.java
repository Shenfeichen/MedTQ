...
public class NTripleMatrixBuilder2Entity {
...
/*	String myDirectoryPath13 = "G:/Bio2RDF datasets/omim";
	...
	SortedMap<String,List<String>> ObjPredMap = new TreeMap<String,List<String>>(); 
	 SortedMap<String,List<String>> SubjPredMap = new TreeMap<String,List<String>>();
	...
	for(int i=0;i<dirList.size();i++){ ////for mix
		...
		if(rowcount>0){	
			...
			/*	if(p.contains("#domain")||p.contains("#range")||p.contains("#import")||p.contains("#subPropertyOf")||p.contains("#isDefinedBy")||p.contains("#disjointWith")||p.contains("#inverseOf")||(p.contains("www.w3.org")&&!p.contains("seeAlso"))||(p.contains("www.w3.org")&&!p.contains("sameAs"))) // temp add for FMA data --- 8/21/2015
				...
				*/
				if(s.contains("#domain")||s.contains("#range")||s.contains("#import")||s.contains("#subPropertyOf")||s.contains("#isDefinedBy")||s.contains("#disjointWith")||s.contains("#inverseOf")||(s.contains("www.w3.org")&&!s.contains("seeAlso"))||(s.contains("www.w3.org")&&!s.contains("sameAs"))) // temp add for FMA data --- 8/21/2015
				{
					continue;
				}
				
				if(o.contains("#domain")||o.contains("#range")||o.contains("#import")||o.contains("#subPropertyOf")||o.contains("#isDefinedBy")||o.contains("#disjointWith")||s.contains("#inverseOf")||(o.contains("www.w3.org")&&!o.contains("seeAlso"))||(o.contains("www.w3.org")&&!o.contains("sameAs"))) // temp add for FMA data --- 8/21/2015
				{
					continue;
				}
				...
				if(dirList.get(i).equals("DBPedia")){
					...
					if(!o.contains("http://dbpedia.org/ontology/"))
					{
						continue;
					}
				}
				...
				/*	if(!p.contains("http://yago-knowledge.org"))
					...
					if(!s.contains("http://yago-knowledge.org"))
					{
						continue;
					}
					if(!o.contains("http://yago-knowledge.org"))
					{
						continue;
					}
				}
	        	...
	        	else
	        	{
			     ...
			     if(!tripleMap.containsKey("s:"+s))
			      ...
			      if(!tripleMap.containsKey("o:"+o))
			      {
					...
					if(!ObjPredMap.containsKey(o))
		        	{
		        		List<String> predicateList = new ArrayList<String>();
		        		predicateList.add(p);
		        		ObjPredMap.put(o,predicateList);
		        	}
		        	else
		        	{
		        		List<String> predicateList = ObjPredMap.get(o);
		        	...
		        	if(!SubjPredMap.containsKey(s))
		        	{
		        		List<String> predicateList = new ArrayList<String>();
		        		predicateList.add(p);
		        		SubjPredMap.put(s,predicateList);
		        	}
		        	else
		        	{
		        		List<String> predicateList = SubjPredMap.get(s);
		        		if(!predicateList.contains(p))
		        		{
		        			predicateList.add(p);
		        		}
...
/*   Iterator<String> iterShareMap2 = ShareMap.keySet().iterator(); 
	...
	{
		HFCMClusterEntity hfcmobj = new HFCMClusterEntity();  
	...
	//	  Graph graph = new Graph();
		...
		//  Graph myGraph= new Graph(columnList.size());
			...
			/*  if(columnList.get(i).contains("subClassOf"))
			  ...
			  for(int j=0;j<columnList.size();j++)
			  {
				...
				//  System.out.println(predicateSubjMap.size());
				  if(SubjPredMap.containsKey(columnList.get(i)))
				  {
					  sbList1 = SubjPredMap.get(columnList.get(i));
					   //domain1=PredicateDomainMap.get(columnList.get(i));
					  // System.out.println("domain1 is: "+domain1);
				  }
				  if(ObjPredMap.containsKey(columnList.get(i)))
				  {
					  obList1 = ObjPredMap.get(columnList.get(i));
					  //   range1=PredicateRangeMap.get(columnList.get(i));
					  // System.out.println("range1 is: "+range1);
				  }
				  if(SubjPredMap.containsKey(columnList.get(j)))
				  {
					  sbList2 = SubjPredMap.get(columnList.get(j));
					 //  domain2=PredicateDomainMap.get(columnList.get(j));
					 //  System.out.println("domain2 is: "+domain2);
				  }
				  if(ObjPredMap.containsKey(columnList.get(j)))
				  {
					  obList2 = ObjPredMap.get(columnList.get(j));
		 ...
		 for(int s=0;s<columnList.size();s++){
			...
			// System.out.println(columnList.get(s));
			 
			 int sDegree=0;
			 int oDegree=0;
			 
			 if(SubjPredMap.containsKey(columnList.get(s)))
			 {
				 sDegree = SubjPredMap.get(columnList.get(s)).size();
			 }
			 
			 if(ObjPredMap.containsKey(columnList.get(s)))
			 {
				 oDegree = ObjPredMap.get(columnList.get(s)).size();
			 }
			 
			 int sumdegree = sDegree+oDegree;
			// int sumdegree = SubjPredMap.get(columnList.get(s)).size()+ObjPredMap.get(columnList.get(s)).size();
		 ...
		 Iterator<String> iterFor2ndNeighbour = neighbourMap.keySet().iterator(); 
		   ...
		   /////// Calculate 3rd level neighbour ///////
			 ...
			 Iterator<String> iterFor3rdNeighbour = neighbourMap2nd.keySet().iterator(); 
			   ...
			   hfcmobj.getMoreMap(neighbourMap,neighbourMap2nd,neighbourMap3rd,SubjPredMap,ObjPredMap,nameNumMap,nameNumMapReverse,subobjMap,secondmap,thirdmap);
			  ...
			  { 
				  ...
				  for(int j=0;j<columnList.size();j++)
				  {
					 ...
					 // System.out.println(columnList.get(i) + " and " + columnList.get(j));
					  ...
					  {
						  if(SubjPredMap.containsKey(columnList.get(i))){
						  List<String> totalSub = SubjPredMap.get(columnList.get(i));
						  neighbourListA.addAll(totalSub);
						 			
						  }
						  
						  if(ObjPredMap.containsKey(columnList.get(i))){
						  List<String> totalObj = ObjPredMap.get(columnList.get(i));
					  ...
					  {
						 if(SubjPredMap.containsKey(columnList.get(j))){
						    List<String> totalSub = SubjPredMap.get(columnList.get(j));
						    neighbourListB.addAll(totalSub);
						 }
						  
						 if(ObjPredMap.containsKey(columnList.get(j))){
							 List<String> totalObj = ObjPredMap.get(columnList.get(j));
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////		    	
		    		  ...
		    		  if(ps==0){
		    			...
		    			for(int u=0;u<secondList.size();u++){			    						    		
    			    			...
    			    			if(SubjPredMap.containsKey(pred1)){
    			    			  List<String> totalSubA = SubjPredMap.get(pred1);
    			    			  neighbourListA.addAll(totalSubA);
    			    			}
    			    			
    			    			if(ObjPredMap.containsKey(pred1)){
    							  List<String> totalObjA = ObjPredMap.get(pred1);
								...
								neighbourListA.addAll(setList);
	    						 ...
	    						 //AB
    			    			  if(SubjPredMap.containsKey(pred2)){
	    						  List<String> SubB = SubjPredMap.get(pred2);
	    						  neighbourListB.addAll(SubB);
    			    			  }
    			    			  
    			    			  if(ObjPredMap.containsKey(pred2)){
	    						  List<String> ObjB = ObjPredMap.get(pred2);
	        					...
	        					//float AB = (2*(float)commonB.size()/((float)neighbourListA.size()+(float)neighbourListB.size()));//Sorensen-Dice coefficient
	    						 ...
	    						 //BC
	    						  if(SubjPredMap.containsKey(pred3)){
	    						  List<String> SubC = SubjPredMap.get(pred3);
	    						  neighbourListC.addAll(SubC);
	    						  }
	    						  
	    						  if(ObjPredMap.containsKey(pred3)){
	    						  List<String> ObjC = ObjPredMap.get(pred3);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
    						...
    						//  float AB = ((float)commonB.size()/((float)neighbourListA.size()+(float)neighbourListB.size()));//jaccard
	    					 ...
	    					 // float BC = (2*(float)commonC.size()/((float)neighbourListB.size()+(float)neighbourListC.size()));//Sorensen-Dice coefficient
	    					  ...
	    					  float AC = AB*BC;
	    					     ...
	    					     if(SubjPredMap.containsKey(pred4)){
	    						  List<String> SubD = SubjPredMap.get(pred4);
	    						  neighbourListD.addAll(SubD);
	    					     }
	    					     
	    					     if(ObjPredMap.containsKey(pred4)){
	    						  List<String> ObjD = ObjPredMap.get(pred4);
			  ...
			  writeCsv2(csvMatrix3,"NeiborCSV/neighborCsv3_entity.csv");
			//!  writeCsv2(csvMatrix3,"/home/user/NeiborCSV/neighborCsv3.csv");
			 ...
			 csvToXLSX("NeiborCSV/neighborCsv3_entity.csv","NeiborCSV/neighborCsv3_entity.xlsx");
	...
	/*	 int row = columnList.size();
		...
		for(int i =0;i<listnames.size();i++){
			...
			if(SubjPredMap.containsKey(key))
			{
				 List<String> value = SubjPredMap.get(key);
	...
	{		
		for(int i =0;i<listnames.size();i++){
			...
			String key = listnames.get(i);
			 ...
			 if(ObjPredMap.containsKey(key))
				{
					 List<String> value = ObjPredMap.get(key);
